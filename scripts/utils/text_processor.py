...
def fix_russian_footnotes(final_text):
    ...
    # Примечание: глобальная конвертация <sup>N</sup> -> [^N] УБРАНА — она ломала
    # математические верхние индексы в теле текста (x¹, β² и т.п.), превращая их
    # в ложные сноски. Подвалы-определения <span...></span><sup>N</sup>Текст
    # собирает сам footnote_def_pattern ниже (ветка <sup>(\d+)</sup>).
